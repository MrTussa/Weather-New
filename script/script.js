...
async function getWeather(lat, lon) {
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=temperature_2m,relativehumidity_2m,weathercode,windspeed_10m&daily=temperature_2m_max,temperature_2m_min,sunrise,sunset&timezone=auto&current_weather=true`;
  try {
...
function hourlyTime(arr, counter) {
  ...
  for (let i = counter + 1; i <= counter + 6; i++) {
    ...
    temperatureData.push({
      x: timeCounter,
      y: Math.round(arr.temperature_2m[i]),
    });
    timeCounter++;
  }
  ...
  for (let i = 0; i <= 6; i++) {
    ...
    <div class="weather-cards__daily">
      ...
      <div class="weather-cards__temp">
        <p class="weather-cards__max">${Math.round(
          arr.temperature_2m_max[i]
        )}°</p>
        <p class="weather-cards__min">${Math.round(
          arr.temperature_2m_min[i]
        )}°</p>
      </div>
...
function drawSmoothChart(temperatureData) {
  ...
  ctx.moveTo(
    ...
    canvas.height -
      ((temperatureData[0].y - minTemp) / tempRange) * canvas.height
  );
  ...
  for (let i = 1; i < temperatureData.length; i++) {
    ...
    const y =
      canvas.height -
      ((temperatureData[i].y - minTemp) / tempRange) * canvas.height +
      1;

    const xPrev =
      ((temperatureData[i - 1].x - minTime) / timeRange) * canvas.width;
    const yPrev =
      canvas.height -
      ((temperatureData[i - 1].y - minTemp) / tempRange) * canvas.height +
      1;
  ...
  ctx.lineTo(
    ((temperatureData[temperatureData.length - 1].x - minTime) / timeRange) *
      canvas.width -
      10,
    canvas.height -
      ((temperatureData[temperatureData.length - 1].y - minTemp) / tempRange) *
        canvas.height -
      10
  );
